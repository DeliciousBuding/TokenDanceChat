...
export function SearchBar() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(async () => {
      try {
        const r = await chatAPI.searchMessages(query.trim());
        setResults(r);
  ...
  return (
    <>
      <div className="td-chat-backdrop fixed inset-0 z-50" onClick={() => setOpen(false)} />
      <div className="fixed inset-x-0 top-[15%] z-50 mx-auto w-full max-w-lg px-4">
        <div className="td-chat-modal overflow-hidden">
          <div className="td-chat-section flex items-center gap-3 border-b px-4 py-3">
            ...
            : <kbd className="td-chat-pill hidden items-center rounded px-1.5 py-0.5 text-[10px] text-muted-foreground/60 sm:inline-flex">ESC</kbd>}
          </div>
          <div className="max-h-72 overflow-y-auto">
            ...
            {noResults && !loading && !error && query && <div className="px-4 py-8 text-center"><Search className="mx-auto h-5 w-5 text-muted-foreground/30 mb-2" /><p className="text-xs text-muted-foreground/50">{t("search.notFound")}</p></div>}
            {scopedResults.length > 0 && scopedResults.map((r, i) => (
              <button key={r.id} onClick={() => handleClickResult(r)}
                className={cn("td-chat-list-row w-full text-left px-4 py-3",
          ...
          <div className="td-chat-section flex items-center gap-2 border-t px-4 py-2">
            <kbd className="td-chat-pill rounded px-1.5 py-0.5 text-[10px] text-muted-foreground/50">Ctrl+K</kbd>
